...
/* Notifier which notifies UART2 TRANSMIT SERVER */
...
/* Name of the server : zx_uart2_rx_server */
void zx_uart2_rx_server()
{ 
   /* ------------------------------------------------------------------*/
   /* Our receive buffer*/
   zx_circular_queue   receive_buffer_uart2;   
   zx_circular_queue*  receive_buffer_uart2_ptr;   
   receive_buffer_uart2_ptr = &receive_buffer_uart2;       
   char receive_buffer_array[ZX_UART2_RX_BUFFER_SIZE];  
      
   /* Initialize this array */
   int i=0;
   for( i=0; i<ZX_UART2_RX_BUFFER_SIZE; i++ )
   {
   ...
   zx_queue_initialize( &receive_buffer_uart2_ptr, receive_buffer_array, 1, ZX_UART2_RX_BUFFER_SIZE);
   
   /* -------------------------------------------------------------------*/
   
   /* Tasks Queue */
   zx_circular_queue   recieve_tasks_queue_uart2;   
   zx_circular_queue*  recieve_tasks_queue_uart2_ptr;  
    
   recieve_tasks_queue_uart2_ptr = &recieve_tasks_queue_uart2;       
   ...
   zx_queue_initialize( &recieve_tasks_queue_uart2_ptr, recieve_tasks_queue_array, 4, TASK_COUNT);
   
   /* -------------------------------------------------------------------*/   
   
   /* Create notifier for this server */
   int notifier_tid = Create(ZX_UART2_RX_NOTIFIER_PRIORITY, &zx_uart2_rx_notifier);
   ...
   FOREVER 
   {
       ...
       { 
           if (to_exit == 0)
           {
               ...
               result = zx_queue_remove( &recieve_tasks_queue_uart2_ptr, clientid_ptr);
               ...
               else
               {
                  ...
                  result = zx_queue_insert( &receive_buffer_uart2_ptr, (void*)byteptr); 
                  /* Reply message for clients */
                  if( result == ZX_ERR_QUEUE_FULL )
                  {
                     zx_printf( COM2, "UART2 Receive Server Error : Buffer Full\n");
                  }
               }
            }
            else
            {
               ...
               while ((result = zx_queue_remove( &recieve_tasks_queue_uart2_ptr, clientid_ptr)) == ZX_Q_SUCCESS)
               {
                  /* We have a client waiting for a byte */
       ...
       else 
       {
           ...
           switch (res.type)
           {
              case API_UART2_GETC:
                  ...
                  result = zx_queue_remove( &receive_buffer_uart2_ptr, byteptr);
                  if ( result == ZX_Q_SUCCESS )
                  {                 
                     /* We have a byte */
                     res.type  = UART_RX_SUCCESS;
                  ...
                  else
                  {
                     ...
                     /* Queue the task */
                     clientid = tid;
                     result = zx_queue_insert( &recieve_tasks_queue_uart2_ptr, clientid_ptr); 
                     /* Reply message for clients */
                     if( result == ZX_ERR_QUEUE_FULL )
                     {
                        res.type  = UART_RX_ERROR;
                        Reply(tid, &res, sizeof(zx_uart_request)); 
                        zx_printf( COM2, "UART2 Receive Server Error : Tasks Queue Full\n");
                     }
                  }
                  break;               
