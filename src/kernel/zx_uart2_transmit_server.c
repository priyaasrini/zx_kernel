...
/* Name of the server : zx_uart2_tx_server */
void zx_uart2_tx_server()
{ 

   /* Our transmit buffer*/
   zx_circular_queue   transmit_queue_uart2;   
   zx_circular_queue*  transmit_queue_uart2_ptr;   
   transmit_queue_uart2_ptr = &transmit_queue_uart2;       
   char transmit_queue_array[ZX_UART2_TX_BUFFER_SIZE];  
   
   /* Initialize this array */
   int i=0;
   for( i=0; i<ZX_UART2_TX_BUFFER_SIZE; i++ )
   {
   ...
   zx_queue_initialize( &transmit_queue_uart2_ptr, transmit_queue_array, 1, ZX_UART2_TX_BUFFER_SIZE);
   
   char byte;
   char *byteptr = &byte;

   /* Create notifier for this server */
   int notifier_tid = Create(ZX_UART2_TX_NOTIFIER_PRIORITY, &zx_uart2_tx_notifier);
   ...
   FOREVER 
   {
       ...
       {   
           ...
           {
               /* Send the byte */
               result = zx_queue_remove( &transmit_queue_uart2_ptr, byteptr);
       ...
       else 
       {
           ...
           switch (res.type)
           {
              case API_UART2_PUTC:
                ...
                {
                   result = zx_queue_insert( &transmit_queue_uart2_ptr, (void*)byteptr); 
...
void zx_uart2_tx_notifier()
{
   ...
   uart_response.type = API_UART2_PUTC;   
